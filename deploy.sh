...
then
  ...
  docker build . -f dev.Dockerfile -t kafkajs-nodejs:local
else
  echo "Building production image ..."  
  docker build . -f Dockerfile -t kafkajs-nodejs:local
fi

echo "Deploying stack"
docker stack deploy $DEPLOYMENT_NAMESPACE --compose-file docker-compose.yml --resolve-image always

docker service logs learning_kafkajs-nodejs -f